...
def get_MedNeXt_model(**kwargs):

    in_channels = 3                             # 输入通道数，灰度图像为 1，RGB 为 3
    n_channels = 32                             # 基础卷积通道数
    n_classes = 8                               # 输出类别数
    exp_r = 2                                   # 扩展比率
    kernel_size = 3                             # 卷积核大小
    deep_supervision = True                     # 启用深度监督
    do_res = True                               # 启用残差连接
    do_res_up_down = True                       # 上下采样使用残差连接
    block_counts = [2,2,2,2,2,2,2,2,2]          # 卷积块数
    checkpoint_style = None                     # 不启用梯度检查点
    dim = '2d'                                  # 使用 2D 数据
    grn = True                                  # 启用 GRN
